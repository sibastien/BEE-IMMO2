  ...
  const cards = testimonials
    .map((testimonial) => `
      <article class="testimonial-card ${testimonial.featured ? 'featured' : ''}">
        ...
        <span>${escapeHtml(testimonial.context || 'Client Bee Immobilier')}</span>
      </article>
    `)
    .join('');
...
const submitContactRequest = async (event) => {
  ...
  try {
    ...
    contactForm.reset();
    setContactMessage('Message envoye. Bee Immobilier vous contactera bientot.');
